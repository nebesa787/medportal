 ...
 */
function wlcenter_customize_register( $wp_customize ) {
	$color_scheme = wlcenter_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'wlcenter_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'wlcenter' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => wlcenter_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'wlcenter' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wlcenter' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'wlcenter' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wlcenter' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wlcenter' );
}
add_action( 'customize_register', 'wlcenter_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'wlcenter_color_schemes'}.
 *
 ...
 */
function wlcenter_get_color_schemes() {
	return apply_filters( 'wlcenter_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'wlcenter' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'wlcenter_get_color_scheme' ) ) :
/**
 ...
 */
function wlcenter_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = wlcenter_get_color_schemes();
...
endif; // wlcenter_get_color_scheme

if ( ! function_exists( 'wlcenter_get_color_scheme_choices' ) ) :
/**
 ...
 */
function wlcenter_get_color_scheme_choices() {
	$color_schemes                = wlcenter_get_color_schemes();
...
endif; // wlcenter_get_color_scheme_choices

if ( ! function_exists( 'wlcenter_sanitize_color_scheme' ) ) :
/**
 ...
 */
function wlcenter_sanitize_color_scheme( $value ) {
	$color_schemes = wlcenter_get_color_scheme_choices();
...
endif; // wlcenter_sanitize_color_scheme
 ...
 */
function wlcenter_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = wlcenter_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = wlcenter_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = wlcenter_get_color_scheme_css( $colors );

	wp_add_inline_style( 'wlcenter-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'wlcenter_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function wlcenter_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', wlcenter_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'wlcenter_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function wlcenter_customize_preview_js() {
	wp_enqueue_script( 'wlcenter-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'wlcenter_customize_preview_js' );
 ...
 */
function wlcenter_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function wlcenter_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-wlcenter-color-scheme">
		<?php echo wlcenter_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'wlcenter_color_scheme_css_template' );
